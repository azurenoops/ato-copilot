...
public class AuditEvidenceCollectorTests
{
    ...
    private readonly Mock<ILogger<AuditEvidenceCollector>> _loggerMock = new();

    private AuditEvidenceCollector CreateCollector() =>
        new(_azureResourceMock.Object, _policyMock.Object, _loggerMock.Object);

    [Fact]
    public void FamilyCode_ShouldBeAU()
    {
        CreateCollector().FamilyCode.Should().Be("AU");
    }
    ...
    [Fact]
    public async Task CollectAsync_ContainsConfigurationEvidence()
    {
        SetupAllServices();

        var result = await CreateCollector().CollectAsync("sub-1", null);

        result.EvidenceItems.Should().Contain(e => e.Type == EvidenceType.Configuration);
    }

    [Fact]
    public async Task CollectAsync_ContainsLogEvidence()
    {
        SetupAllServices();
    ...
    public async Task CollectAsync_HasAttestation()
    {
        ...
        result.AttestationStatement.Should().Contain("Audit");
    }
