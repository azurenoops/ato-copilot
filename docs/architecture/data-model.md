...
Per-control SSP narrative with implementation status. Unique composite index on `(SystemId, ControlId)`.
...
| `SystemId` | `Guid` | FK → RegisteredSystem |
| `ControlId` | `string` | NIST control ID |
| `Narrative` | `string` | Implementation narrative |
...
| `ComplianceFinding` | Individual non-compliant finding with severity, status |
